...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
    ...
    const int keyboardsSize = keyboards.size();
    const int drivesSize = drives.size();

    int result = -1;

    for(int i = 0; i < keyboardsSize; i++)
    {
	    for(int j = 0; j < drivesSize; j++)
	    {
            const int budget = keyboards[i] + drives[j];
            if(budget <= b)
            {
                result = max(result, budget);
            }
	    }
    }
